...
// 
// 
public class MyCrosshair
{
	public static MyCrosshair instance;
	
	#region Variables
	public Texture ImageCrosshair 	  = null;
	
	public OVRCameraController CameraController = null;
	public MoveController playerController = null;
	...
	//public GameObject debugCrosshair;
	...
	public void SetOVRCameraController(ref OVRCameraController cameraController)
	{
		...
		//debugCrosshair.transform.parent = MainCam.transform;
	}

	// SetOVRPlayerController
	public void SetOVRPlayerController(ref MoveController controller)
	{
	...
	public void Init()
	{
		...
		//debugCrosshair = GameObject.CreatePrimitive(PrimitiveType.Plane);
		//debugCrosshair.transform.localScale = new Vector3(.1f, .1f, .1f);
		//debugCrosshair.transform.rotation = Quaternion.Euler(-90, 0, 0);
		
		//debugCrosshair.GetComponent<MeshRenderer>().material.mainTexture = ImageCrosshair;
		//GameObject.Destroy(debugCrosshair.GetComponent<MeshCollider>());
	}
	...
	public void UpdateCrosshair()
	{
		...
		//debugCrosshair.transform.position = aimPosition;
	}
	...
	public void  OnGUICrosshair()
	{
		...
		if(playerController != null)
			playerController.AllowRotation(false);
		...
		if ((ImageCrosshair != null) && (FadeVal != 0.0f))
		{
			...
			if(playerController != null)
				playerController.AllowRotation(true);
			...
			//if(allowMouseRotation == true)
			{
				...
				*/
				...
				aimPosition = MainCam.ScreenToWorldPoint (new Vector3 ((XL * 1.5f - (ImageCrosshair.width * 0.5f) + xOffset )
					,Screen.height - (YL - (ImageCrosshair.height * 0.5f)) + yOffset , CrosshairDistance + 1));
